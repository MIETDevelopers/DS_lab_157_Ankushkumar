...
struct node *head;  //create aself referention stucture
...
void insertion_beginning()  //insert function
{  
   struct node *ptr;   
   int item;  
   ptr = (struct node *)malloc(sizeof(struct node));//memory allocated  
   if(ptr == NULL)  //if ptr equal to Null so it is true print this 
   {  
       printf("\nOVERFLOW");  
   }  
   else  
   ...
   {  
       ...
       ptr->next = head;  
       head->prev=ptr;  
       head=ptr;  //Ptr assiged to head
   }  
   printf("\nNode inserted\n");  //And print node insert
}  
     
}  
void insertion_last()  //Insert function
{  
   ...
   if(ptr == NULL)  
   {  
       printf("\nOVERFLOW");  //ovetflow if condtion true
   }  
   else  
   {  
       printf("\nEnter value");  
       scanf("%d",&item);  
        ptr->data=item;  //Item to ptr
       if(head == NULL)  
       {  
           ptr->next = NULL;  
           ptr->prev = NULL;  
